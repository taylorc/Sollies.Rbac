...
namespace Sollies.Rbac.Shared.DataAccess
{
        public class RetrieveData : IRetrieveData
        {
            ...
            public async Task<List<JObject>> GetPermsetUserPerms(string permsetId, HashSet<string> userPerms, bool retry)
            {
                string cacheKey = "-userperms-" + permsetId;

                List<JObject> userPermCacheString = memoryCache.Get(cacheKey) as List<JObject>;
